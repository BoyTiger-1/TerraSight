...
async function assess(loc) {
  ...
  const rows = Object.entries(data.results)
    ...
    .map(([slug, r]) => ({ slug, score: r.assessment.score, level: r.assessment.level,
      ...
      coupled: r.cascades && r.cascades.score_after > r.cascades.score_before ? r.cascades : null,
      grid: (r.data_provenance || {}).national_grid_fallback,
      sub: (r.data_provenance || {}).substituted_location }))
    .sort((a, b) => b.score - a.score);

  matrixBody.innerHTML = "";
  rows.forEach(({ slug, score, level, title, coupled, sub, grid }) => {
    ...
    // the score shown is always the module's own model, which is what the
    // module page shows too; coupling is an overlay and says so.
    const tags = (coupled ? ` <span class="coupled-tag" title="cross-hazard coupling would carry this to ${coupled.score_after.toFixed(0)} (${coupled.incoming.map(e => "+" + e.boost + " from " + e.source).join(", ")}). The number shown is this module's own model.">coupled &rarr; ${coupled.score_after.toFixed(0)}</span>` : "")
